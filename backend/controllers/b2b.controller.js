...
class B2BAnalyticsController {
  ...
  async getBenchmarkingAnalytics(req, res, next) {
    try {
      ...
      if (industry) {
        ...
        const companies = await Company.find({ industry }).select('_id');
        const companyIds = companies.map(c => c._id);
        
        industryFilter = { company: { $in: companyIds } };
      }
      
      // Average time-to-fill (job posting to hiring)
      const timeToFill = await Application.aggregate([
        { $match: { status: "hired" } }, // Only look at successful hires
        {
          $lookup: {
            from: "jobs",
            localField: "job",
        ...
        { $unwind: "$jobData" },
        { $match: industryFilter }, // Apply industry filter if provided
        {
          $project: {
            _id: 0,
            timeToFill: {
              $subtract: ["$updatedAt", "$jobData.createdAt"] // Time between job posting and hire
            }
          }
        },
        {
          $group: {
            _id: null,
            averageTimeToFill: { $avg: "$timeToFill" }, // In milliseconds
        ...
        },
        {
          $project: {
            _id: 0,
            averageTimeToFillDays: { $divide: ["$averageTimeToFill", 1000 * 60 * 60 * 24] }, // Convert to days
            minTimeToFillDays: { $divide: ["$minTimeToFill", 1000 * 60 * 60 * 24] },
            maxTimeToFillDays: { $divide: ["$maxTimeToFill", 1000 * 60 * 60 * 24] }
          }
        }
      ]);
      ...
      const successRate = await Application.aggregate([
        ...
        },
        {
          $project: {
            ...
            successRate: {
              $divide: [
                { $ifNull: [{ $arrayElemAt: ["$statuses.count", { $indexOfArray: ["$statuses.status", "hired"] }] }, 0] },
                "$totalApplications"
              ]
            },
      ...
      return res.status(200).json({
        success: true,
        data: {
          timeToFill: timeToFill[0] || { averageTimeToFillDays: 0 },
          successRate: successRate[0] || { successRate: 0, totalApplications: 0 },
          industryFiltered: !!industry
        }
      });
    } catch (error) {
      next(error);
